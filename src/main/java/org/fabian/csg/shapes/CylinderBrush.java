...
public class CylinderBrush extends CSG {
	...
	private Vertex point(int stack, float slice, float normalBlend)
	{
		...
		Vector3f out = axisX.mult((float) Math.cos(angle))
				.addLocal(axisY.mult((float) Math.sin(angle)));
		Vector3f pos = start.add(ray.mult(stack)).add(out.mult(radius));
		Vector3f normal = out.mult(1 - Math.abs(normalBlend))
				.add(axisZ.mult(normalBlend));
	...
	public CylinderBrush(Vector3f start, Vector3f end, float radius, int slices)
	{
		...
		axisX = new Vector3f(isY ? 1 : 0, isY ? 0 : 1, 0).cross(axisZ)
				.normalizeLocal();
		...
		for (int i = 0; i < slices; i++)
		{
			...
			polygons.add(new Polygon(
					new Vertex[] { startV, point(0, t0, -1), point(0, t1, -1) },
					null));
			polygons.add(new Polygon(new Vertex[] { point(0, t1, 0),
					point(0, t0, 0), point(1, t0, 0), point(1, t1, 0) }, null));
			polygons.add(new Polygon(
					new Vertex[] { endV, point(1, t1, 1), point(1, t0, 1) },
					null));
		}
